...
const CarKeyFeatures = ({ item }) => {
  ...
  const specs = {
    ...
    speed: item?.keySpecifications?.topSpeed ? (item.keySpecifications.topSpeed?.toLowerCase()?.includes('mh') || item.keySpecifications.topSpeed?.toLowerCase()?.includes('km') ? item.keySpecifications.topSpeed : `${item.keySpecifications.topSpeed} mph`) : (item?.keySpecifications?.mileage || "300 km/h"),
    engine: item?.keySpecifications?.engineType || item?.keySpecifications?.cylinderCapacity || "Inline V6"
  };
