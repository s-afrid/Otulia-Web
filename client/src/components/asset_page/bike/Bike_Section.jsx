...
const Bike_Section = () => {
    ...
    const fetchSimilar = async (assetId) => {
        try {
            const response = await fetch(`/api/assets/similar/bike/${assetId}`);
            if (response.ok) {
    ...
    const fetchAgentAssets = async (agentId, currentId) => {
        try {
            const response = await fetch(`/api/assets/agent/${agentId}/bike?excludeId=${currentId}`);
            if (response.ok) {
                const data = await response.json();
                setAgentAssets(data);
            }
           ...
           <SEO 
            ...
            {similarAssets.length > 0 && (
                <div className="py-8 bg-gray-50/50">
                    <AssetSlider title="Similar Bikes" items={similarAssets} />
                </div>
            )}
