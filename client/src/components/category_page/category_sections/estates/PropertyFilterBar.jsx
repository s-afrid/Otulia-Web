    ...
    return (
        ...
        "
            >
                ...
                whitespace-nowrap
                ${(selectedFilters[filter.key] && !selectedFilters[filter.key].startsWith('Any')) || activeFilter === filter.label
              ...
              `}
                        ...
                        {activeFilter === filter.label && (
                            <>
                                <div className="fixed inset-0 z-10" onClick={() => setActiveFilter(null)}></div>
                                <div className="absolute top-full left-0 mt-2 w-48 bg-white border border-gray-100 rounded-xl shadow-2xl z-20 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                                    {filter.options.map((option, idx) => (
                                        <div
                                            key={idx}
                                            className={`px-4 py-3 text-sm cursor-pointer transition-colors ${selectedFilters[filter.key] === option ? 'bg-gray-100 text-black font-bold' : 'text-gray-600 hover:bg-gray-50 hover:text-black'}`}
