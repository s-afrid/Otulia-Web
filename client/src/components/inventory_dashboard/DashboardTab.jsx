...
import { FiTrendingUp, FiTrendingDown, FiEye, FiUser, FiHeart, FiArrowRight, FiActivity, FiChevronDown } from 'react-icons/fi';
import numberWithCommas from '../../modules/numberwithcomma';

const DashboardTab = ({ data, generateSparkline, chartInterval, setChartInterval, getSparklineData, convInterval, setConvInterval, setActiveTab }) => {
    return (
                                    ...
                                    <tr key={i} className="hover:bg-gray-50/30 transition-colors">
                                        <td className="py-[clamp(6px,1.2vh,20px)] flex items-center gap-[clamp(6px,0.8vh,16px)] truncate">
                                            <div className="w-[clamp(28px,4vh,60px)] h-[clamp(18px,2.5vh,40px)] rounded-[clamp(4px,0.6vh,12px)] bg-[#F3EBE3] shrink-0 flex items-center justify-center overflow-hidden">
                                                {item.image ? <img src={item.image} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gray-100 flex items-center justify-center text-gray-300">Image</div>}
                                            </div>
