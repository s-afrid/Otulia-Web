    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 lg:p-10">
      <div className="bg-white rounded-[clamp(12px,2vh,32px)] shadow-2xl w-full h-full max-w-5xl flex flex-col relative overflow-hidden animate-in zoom-in duration-300">
        ...
        >
          {/* Header */}
          <div
            className="flex justify-between items-center p-[clamp(16px,2.5vh,40px)] border-b"
            style={{ borderColor: "#f3f4f6", backgroundColor: "#ffffff" }}
          >
            <div className="flex items-center gap-4">
              <div
                className="w-[clamp(32px,4vh,64px)] h-[clamp(32px,4vh,64px)] rounded-[clamp(8px,1.2vh,20px)] flex items-center justify-center border"
                style={{ backgroundColor: "#eff6ff", borderColor: "#f8fafc" }}
              >
                <FiPieChart
                  className="text-[clamp(16px,2vh,32px)]"
                  style={{ color: "#2563eb" }}
                />
              </div>
              <div>
                <h2
                  className="text-[clamp(18px,2.4vh,36px)] font-bold kaisei"
                  style={{ color: "#111827" }}
                >
                  Leads Source Analysis
                </h2>
                <p
                  className="inter text-[clamp(10px,1.3vh,18px)] font-medium uppercase tracking-[0.05em]"
                  style={{ color: "#6b7280" }}
                >
                  Detailed breakdown of lead acquisition channels
                </p>
              </div>
            </div>
            <button
          ...
          <div className="flex-1 flex flex-col lg:flex-row min-h-0">
            ...
            >
              <div className="w-full max-w-md aspect-square relative flex items-center justify-center">
                ...
                >
                  ...
                  {(() => {
                    const total =
                      data.reduce((s, i) => s + (i.count || 0), 0) || 1;
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                  <span
                    className="text-5xl font-bold leading-none kaisei"
                    style={{ color: "#111827" }}
                  >
                    {numberWithCommas(totalLeads)}
                  </span>
                  <span
                    className="inter text-sm font-bold uppercase tracking-widest mt-2"
                    style={{ color: "#9ca3af" }}
                  >
                    Total Leads
                  </span>
            ...
            {/* Table Side */}
            <div
              className="flex-1 border-l flex flex-col min-h-0"
              style={{ borderLeftColor: "#f3f4f6", backgroundColor: "#ffffff" }}
            >
              <div className="p-8 shrink-0">
                <h3
                  className="text-lg font-bold mb-2 inter"
                  style={{ color: "#111827" }}
                >
              ...
              <div className="flex-1 overflow-y-auto px-8 pb-8 custom-scrollbar">
                <div className="space-y-6">
                  {data.map((src, idx) => {
                    const color = colorMap[src.label] || "#9CA3AF";
                    const iconInfo = iconMap[src.label] || {
                      ...
                      <div key={idx} className="group">
                        <div className="flex justify-between items-center mb-3">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <span
                                className="block font-bold inter"
                                style={{ color: "#1f2937" }}
                              >
                                {src.label}
                              </span>
        ...
        {/* Action Footer */}
        <div
          className="p-8 border-t shrink-0 no-export"
        ...
        >
          <button
            ...
            style={{
              ...
              boxShadow: "0 10px 15px -3px rgba(30, 59, 112, 0.2)",
            }}
          >
            {isExporting ? (
              <>
