                    ...
                    {activeTab === 'dashboard' && (
                        <div className="h-[calc(100vh-6rem-5rem)] flex flex-col gap-4 animate-in fade-in duration-700 pb-2">
                            ...
                            <div className="flex justify-between items-end shrink-0 mb-1">
                                <div>
                                    <h2 className="text-xl font-bold text-gray-900 font-playfair mb-0 leading-tight">Welcome back, {user?.name?.split(' ')[0] || user?.name || 'User'} <span className="text-[18px]">👋</span></h2>
                                </div>
                            </div>
                                ...
                                <div className="bg-white rounded-[1.25rem] p-4 flex flex-col justify-between border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)] relative overflow-hidden flex-1 group">
                                    <div className="flex justify-between items-start z-10">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-2xl font-bold text-gray-900 font-playfair leading-none mt-1">{numberWithCommas(data?.stats?.totalViews || 0)}</span>
                                            <span className="text-[9px] font-bold text-emerald-500 flex items-center gap-1 mt-1.5 tracking-wide"><FiTrendingUp className="text-[10px]" /> 0.0% <span className="text-gray-400/80 font-medium">vs last 30 days</span></span>
                                        </div>
                                ...
                                <div className="bg-white rounded-[1.25rem] p-4 flex flex-col justify-between border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)] relative overflow-hidden flex-1 group">
                                    <div className="flex justify-between items-start z-10">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-2xl font-bold text-gray-900 font-playfair leading-none mt-1">{data?.stats?.savedCount || 0}</span>
                                            <span className="text-[9px] font-bold text-emerald-500 flex items-center gap-1 mt-1.5 tracking-wide"><FiTrendingUp className="text-[10px]" /> 0.0% <span className="text-gray-400/80 font-medium">vs last 30 days</span></span>
                                        </div>
                                ...
                                <div className="bg-white rounded-[1.25rem] p-4 flex flex-col justify-between border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)] relative overflow-hidden flex-1 group">
                                    <div className="flex justify-between items-start z-10">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-2xl font-bold text-gray-900 font-playfair leading-none mt-1">${(data?.stats?.estLeadValue || 0) >= 1000000 ? ((data?.stats?.estLeadValue || 0)/1000000).toFixed(2) + 'M' : numberWithCommas(data?.stats?.estLeadValue || 0)}</span>
                                            <span className="text-[9px] font-bold text-emerald-500 flex items-center gap-1 mt-1.5 tracking-wide"><FiTrendingUp className="text-[10px]" /> 0.0% <span className="text-gray-400/80 font-medium">vs last 30 days</span></span>
                                        </div>
                                    ...
                                    <div className="flex-1 flex gap-2 items-center relative">
                                        <div className="w-1/2 h-full flex items-center justify-center relative scale-[1.3] translate-x-2">
                                            ...
                                            <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                                                <span className="text-[20px] font-bold text-gray-900 leading-none">{data?.stats?.totalLeads || 0}</span>
                                        ...
                                        <div className="w-1/2 flex flex-col justify-center gap-2.5 z-10 pl-6">
                                            {[ 
                                                {n:'Cars', v: (data?.leads || []).filter(l=>l.category==='CarAsset' || l.category==='vehicles').length, c:'#D48D2A'}, 
                                                {n:'Yachts', v: (data?.leads || []).filter(l=>l.category==='YachtAsset' || l.category==='yachts').length, c:'#1E3B70'},
                                                {n:'Real Estate', v: (data?.leads || []).filter(l=>l.category==='EstateAsset' || l.category==='estates').length, c:'#10B981'},
                                                {n:'Others', v: (data?.leads || []).filter(l=>!['CarAsset','vehicles','YachtAsset','yachts','EstateAsset','estates'].includes(l.category)).length, c:'#9CA3AF'}
                                            ].map((r,i) => {
                                                const total = (data?.stats?.totalLeads || 1) === 0 ? 1 : data?.stats?.totalLeads || 1;
                                                ...
                                                <div key={i} className="flex items-center justify-between text-[9px] font-bold">
                                                    ...
                                                    <span className="text-gray-900 truncate pl-1 flex gap-1"><span className="w-3 text-right">{r.v}</span> <span className="text-gray-400/80 font-medium w-6 text-right">({p})</span></span>
                                                </div>
                                            )})}
                                        </div>
                                    </div>
                                    <div className="flex justify-end pt-1 shrink-0 mt-2">
                                        <button onClick={() => setActiveTab('analytics')} className="text-[9px] font-bold text-[#D48D2A] hover:bg-[#FFF8F0] px-2 py-1 rounded transition-colors uppercase tracking-widest flex items-center gap-1">View Full Report <FiChevronRight/></button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-4 shrink-0 h-[170px]">
                                ...
                                <div className="w-5/12 bg-white rounded-[1.25rem] p-4 flex flex-col border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)] overflow-hidden">
                                    <div className="flex justify-between items-center mb-2">
                                        ...
                                        <button onClick={() => setActiveTab('inventory')} className="text-[9px] font-bold text-[#D48D2A] bg-[#FFF8F0] px-2 py-1 rounded transition-colors uppercase tracking-widest">View all</button>
                                    </div>
                                        ...
                                        <table className="w-full text-left table-fixed">
                                            <thead className="sticky top-0 bg-white z-10">
                                                <tr>
                                                    ...
                                                    <th className="pb-1.5 pt-1 text-[8px] font-black text-gray-400/80 uppercase tracking-widest border-b border-gray-50/50 w-3/12 text-center">Views</th>
                                                    <th className="pb-1.5 pt-1 text-[8px] font-black text-gray-400/80 uppercase tracking-widest border-b border-gray-50/50 w-2/12 text-right pr-2">Leads</th>
                                                </tr>
                                            </thead>
                                                ...
                                                {(data?.inventory || []).sort((a,b) => (b.views || 0) - (a.views || 0)).slice(0, 4).map((item, i) => (
                                                    <tr key={i} className="hover:bg-gray-50/50">
                                                        <td className="py-2 flex items-center gap-2 truncate">
                                                            <div className="w-6 h-6 rounded object-cover shadow-sm border border-gray-200/50 shrink-0 bg-gray-100 flex items-center justify-center overflow-hidden">
                                                                {item.images?.[0] ? <img src={item.images[0]} className="w-full h-full object-cover"/> : <FiImage className="text-gray-300"/>}
                                                            </div>
                                                            <span className="text-gray-900 truncate overflow-hidden whitespace-nowrap">{item.name || `${item.make || ''} ${item.model || ''}`}</span>
                                                        </td>
                                                        <td className="py-2 text-center text-gray-600 font-black">{item.views || 0}</td>
                                                        <td className="py-2 text-right text-[#D48D2A] pr-2 tracking-wide flex justify-end items-center"><FiUsers className="mr-1 text-[8px]"/>{item.leads || 0}</td>
                                                    </tr>
                                                ))}
                                                {(!data?.inventory || data?.inventory?.length === 0) && (
                                                    <tr><td colSpan="3" className="py-4 text-center text-xs font-medium text-gray-400">No assets available</td></tr>
                                                )}
                                            </tbody>
                                        </table>
                                    </div>
                                ...
                                <div className="w-4/12 bg-white rounded-[1.25rem] p-4 flex flex-col border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)] relative">
                                    <div className="flex justify-between items-center mb-0">
                                        <h4 className="text-[14px] font-bold text-gray-900 font-playfair tracking-wide">Leads By Status</h4>
                                    </div>
                                    <div className="flex-1 flex items-center justify-between z-10 px-0">
                                        ...
                                        <div className="w-1/2 flex flex-col justify-center gap-1.5 pl-2 z-10 pb-2">
                                            {[
                                                {n:'New', v: (data?.leads || []).filter(l=>l.status==='New').length, c:'#D48D2A'}, 
                                                {n:'Qualified', v: (data?.leads || []).filter(l=>l.status==='Qualified').length, c:'#1E3B70'}, 
                                                {n:'Contacted', v: (data?.leads || []).filter(l=>l.status==='Contacted').length, c:'#10B981'}, 
                                                {n:'Closed', v: (data?.leads || []).filter(l=>l.status==='Closed').length, c:'#8B5CF6'}, 
                                                {n:'Lost', v: (data?.leads || []).filter(l=>l.status==='Lost').length, c:'#9CA3AF'}
                                            ].map((r,i) => {
                                                const total = (data?.stats?.totalLeads || 1) === 0 ? 1 : data?.stats?.totalLeads || 1;
                                                const p = ((r.v / total) * 100).toFixed(1) + '%';
                                                return (
                                                <div key={i} className="flex items-center justify-between text-[8px] font-bold">
                                                    ...
                                                    <span className="text-gray-900 text-right flex shrink-0 whitespace-nowrap"><span className="w-[14px]">{r.v}</span> <span className="text-gray-400 font-medium w-[24px] tracking-tight text-right">({p})</span></span>
                                                </div>
                                            )})}
                                        </div>
                                    </div>
                                ...
                                <div className="w-3/12 bg-white rounded-[1.25rem] p-4 flex flex-col border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)]">
                                    <div className="flex justify-between items-start mb-0">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[18px] font-bold text-gray-900 mt-1">{data?.stats?.totalViews ? ((data?.stats?.totalLeads / data?.stats?.totalViews) * 100).toFixed(1) : '0.0'}%</span>
                                        </div>
                                    </div>
                            ...
                            <div className="flex gap-4 shrink-0 h-[100px]">
                                <div className="w-[58%] bg-white rounded-[1.25rem] p-4 flex flex-col border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)] justify-between h-[100px]">
                                    <div className="flex justify-between items-center mb-1">
                                        <h4 className="text-[14px] font-bold text-gray-900 font-playfair tracking-wide leading-none">Recent Activity</h4>
                                    </div>
                                    <div className="space-y-[5px] flex-1 overflow-auto custom-scrollbar pr-2">
                                        {(data?.notifications || []).slice(0, 3).map((notif, idx) => (
                                            <div key={idx} className="flex justify-between items-center text-[10px]">
                                                <div className="flex items-center gap-2 text-gray-600 truncate"><FiBell className="text-gray-400 shrink-0"/> <span className="font-medium truncate">{notif.message}</span></div>
                                                <span className="text-gray-400 font-medium shrink-0 ml-2">{new Date(notif.createdAt).toLocaleDateString()}</span>
                                            </div>
                                        ))}
                                        {(!data?.notifications || data.notifications.length === 0) && (
                                            <div className="text-[10px] text-gray-400 font-medium text-center py-2 italic mt-4">No recent activity detected.</div>
                                        )}
                                    </div>
                                </div>
                                <div className="w-[42%] bg-white rounded-[1.25rem] p-4 flex flex-col border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)] justify-between h-[100px]">
                                    <div className="flex justify-between items-center mb-2">
                                        <h4 className="text-[14px] font-bold text-gray-900 font-playfair tracking-wide leading-none">Assets Overview</h4>
                                        <button onClick={() => setActiveTab('inventory')} className="text-[9px] font-bold text-gray-700 border border-gray-200 shadow-sm bg-gray-50 px-2 py-1.5 rounded transition-colors uppercase tracking-widest flex items-center hover:bg-gray-100">Manage Assets</button>
                                    </div>
                                    <div className="flex justify-between items-start mt-2 px-1">
                                        <div className="flex flex-col text-center">
                                            <span className="text-[8px] font-black uppercase tracking-widest text-gray-400 mb-1">Total Assets</span>
                                            <span className="text-[22px] font-bold text-gray-900 leading-none">{(data?.inventory || []).length}</span>
                                        </div>
                                        ...
                                        <div className="flex flex-col text-center">
                                            ...
                                            <span className="text-[20px] font-bold text-gray-900 leading-none">{(data?.inventory || []).filter(a=>a.status==='Active').length}</span>
                                        ...
                                        <div className="flex flex-col text-center">
                                            ...
                                            <span className="text-[20px] font-bold text-gray-900 leading-none">{(data?.inventory || []).filter(a=>a.status==='Draft').length}</span>
                                        ...
                                        <div className="flex flex-col text-center pr-2">
                                            ...
                                            <span className="text-[20px] font-bold text-gray-900 leading-none">{(data?.inventory || []).filter(a=>a.status==='Sold').length}</span>
...
\n"""
    ...
    with open('/Users/sudeepreddy/Documents/otulia/Otulia-Web/client/src/pages/Inventory.jsx', 'w') as f:
        f.write(content)
        print("Success Dashboard Dynamic")
else:
    print("Fail Dashboard Dynamic")
